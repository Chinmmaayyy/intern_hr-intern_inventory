...
const INVENTORY_NAV_SECTIONS: NavSection[] = [
    {
        title: 'Overview',
        items: [
    ...
    },
    {
        ...
        collapsible: true,
        items: [
            { label: 'Stores & Stock', href: '/inventory/stores', icon: Store },
    ...
    },
    {
        ...
        items: [
            ...
            { label: 'Procurement Hub', href: '/inventory/procurement', icon: ShoppingCart },
        ],
    },
    {
        title: 'Reports',
        items: [
            { label: 'Reports & MIS', href: '/inventory/reports', icon: BarChart3 },
        ],
    },
];

export default function InventorySidebar({ userRole }: { userRole?: string }) {
    ...
    useEffect(() => {
        const saved = window.localStorage.getItem('inventory-sidebar-collapsed');
        if (saved === 'true') setCollapsed(true);
        const savedSections = window.localStorage.getItem('inventory-sidebar-sections');
        if (savedSections) {
    ...
    useEffect(() => {
        window.localStorage.setItem('inventory-sidebar-collapsed', String(collapsed));
    }, [collapsed]);
    ...
    const toggleSection = (title: string) => {
        setCollapsedSections((prev) => {
            ...
            window.localStorage.setItem('inventory-sidebar-sections', JSON.stringify([...next]));
            return next;
        });
    };

    const isActive = (href: string) => {
        if (href === '/inventory/dashboard') return pathname === href || pathname === '/inventory';
    ...
    const sidebarContent = (
        ...
        >
            ...
            >
                ...
                {!collapsed && (
                    <div className="overflow-hidden">
                        <p className="text-[10px] text-gray-500 tracking-wider font-medium">INVENTORY</p>
                    </div>
                )}
                <button
            ...
            <nav className="flex-1 overflow-y-auto py-4 px-2.5 space-y-5">
                {INVENTORY_NAV_SECTIONS.map((section) => {
