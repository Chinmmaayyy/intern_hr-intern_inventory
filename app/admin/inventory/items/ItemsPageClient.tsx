...
export default function ItemsPageClient({ userRole }: { userRole?: string }) {
  const canApproveItems = userRole === 'admin';
                ...
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      {item.status === 'Draft' && canApproveItems && (
                      ...
                      {item.status === 'Active' && canApproveItems && (
